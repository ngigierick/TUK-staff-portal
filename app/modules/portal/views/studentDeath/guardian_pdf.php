...
<div class="wrapper">
	...
	<h2 class="upper center underline">BEREAVEMENT - <?php echo $model->regNumber->surname.' '.$model->regNumber->firstname.' '.$model->regNumber->othername;?> REG NO. <?php echo $model->regNumber->reg_number;?> </h2>
	</center>
	
	<p><b><?php echo $model->regNumber->title.' '.$model->regNumber->surname.' '.$model->regNumber->firstname.' '.$model->regNumber->othername;?>,</b> a student at the school of <b><?php echo $model->regNumber->programme->department->school;?></b> 
		in the department of <b><?php echo $model->regNumber->programme->department;?> </b>with the Registration Number <b><?php echo $model->reg_number;?></b> 
		has lost <?php echo ($model->regNumber->gender_id==1)?'his ':'her ';?>  <b><?php echo strtolower($model->relation);?>,<?php echo $model->affected_name;?></b>.</p>
	<p>The burial will be held on <?php echo date('l, jS of F Y', strtotime($model->burial_date));?> at <?php echo $model->burial_location;?>.</p>
	...
	<p> We wish to send heartfelt condolences to <?php echo ($model->regNumber->gender_id==1)?'his ':'her ';?> family, relatives and friends.</p>
