...
class EmployeeProjectController extends GxController {
...
public $pageTitle = ":: Staff ePortal";
	...
	public function actionView($id) {
		if (!Yii::app()->getRequest()->getIsAjaxRequest())
			$this->render('view', array(
				'model' => $this->loadModel($id, 'EmployeeProject'),
			));
		else
	...
	public function actionCreate() {
		...
		if (isset($_POST['EmployeeProject'])) {
			$model->setAttributes($_POST['EmployeeProject']);
			...
			//$model->home=$model->country;
				...
				if ($model->save()) {
					...
					$activity->user_cat =4;
					$activity->details = "staff added project details ";
					$activity->date_recorded = time();
					$activity->save();
					...
					$session['page'] = 6;
					 ...
					 if (Yii::app()->request->isAjaxRequest)
	                 {
	                    echo CJSON::encode(array(
	                        'status'=>'success', 
	                        'div'=>" Project details updated successfully!"
	                        ));
	                    exit;               
	                 }
						...
						Yii::app()->user->setFlash('success', '<b>New project details added successfully!</b>');
						$this->redirect(Yii::app()->createUrl('//portal/profile/view'));
					 }
	...
	public function actionUpdate($id) {
		...
		if (isset($_POST['EmployeeProject'])) {
			...
			if ($model->save()) {
				...
				$activity->theuser = $staff->id;
				$activity->user_cat =4;
				$activity->details = "staff updated project details ";
				$activity->date_recorded = time();
				$activity->save();
				 ...
				 if (Yii::app()->request->isAjaxRequest)
                 {
					...
					Yii::app()->user->setFlash('success', '<b>Project details updated successfully!</b>');
					$this->redirect(Yii::app()->createUrl('//portal/profile/view'));
				 }
	...
	public function actionDelete1($id) {
		if (Yii::app()->getRequest()->getIsPostRequest()) {
			$this->loadModel($id, 'EmployeeProject')->delete();
	...
	public function actionDelete($id) {
		...
		if ($this->loadModel($id, 'EmployeeProject')->delete()){
			
			$session=new CHttpSession;
			$session->open();
			$session['page'] = 6;
			Yii::app()->user->setFlash('success', '<b>Project deleted successfully!</b>');
			$this->redirect(Yii::app()->createUrl('//portal/profile/view'));
		}
	...
	public function actionIndex() {
		...
		if (isset($_GET['EmployeeProject']))
			$model->setAttributes($_GET['EmployeeProject']);
