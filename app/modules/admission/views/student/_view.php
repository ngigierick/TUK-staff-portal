<div class="view">

	<?php echo GxHtml::encode($data->getAttributeLabel('id')); ?>:
	<?php echo GxHtml::link(GxHtml::encode($data->id), array('view', 'id' => $data->id)); ?>
	<br />

	<?php echo GxHtml::encode($data->getAttributeLabel('reg_number')); ?>:
	<?php echo GxHtml::encode($data->reg_number); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('college_number')); ?>:
	<?php echo GxHtml::encode($data->college_number); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('academicyear_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->academicyear)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('semester_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->semester)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('title_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->title)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('id_number')); ?>:
	<?php echo GxHtml::encode($data->id_number); ?>
	<br />
	<?php /*
	<?php echo GxHtml::encode($data->getAttributeLabel('surname')); ?>:
	<?php echo GxHtml::encode($data->surname); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('firstname')); ?>:
	<?php echo GxHtml::encode($data->firstname); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('othername')); ?>:
	<?php echo GxHtml::encode($data->othername); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('programme_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->programme)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('dob')); ?>:
	<?php echo GxHtml::encode($data->dob); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('gender_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->gender)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('marital_status_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->maritalStatus)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('nationality_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->nationality)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('county_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->county)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('district_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->district)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('ethnicity_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->ethnicity)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('religion_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->religion)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('postal_address')); ?>:
	<?php echo GxHtml::encode($data->postal_address); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('postcode')); ?>:
	<?php echo GxHtml::encode($data->postcode); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('town')); ?>:
	<?php echo GxHtml::encode($data->town); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('mobile')); ?>:
	<?php echo GxHtml::encode($data->mobile); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('email')); ?>:
	<?php echo GxHtml::encode($data->email); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('c_postal_address')); ?>:
	<?php echo GxHtml::encode($data->c_postal_address); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('c_postcode')); ?>:
	<?php echo GxHtml::encode($data->c_postcode); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('c_town')); ?>:
	<?php echo GxHtml::encode($data->c_town); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('c_mobile')); ?>:
	<?php echo GxHtml::encode($data->c_mobile); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('c_email')); ?>:
	<?php echo GxHtml::encode($data->c_email); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('nok_surname')); ?>:
	<?php echo GxHtml::encode($data->nok_surname); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('nok_firstname')); ?>:
	<?php echo GxHtml::encode($data->nok_firstname); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('nok_othername')); ?>:
	<?php echo GxHtml::encode($data->nok_othername); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('nok_title_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->nokTitle)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('nok_relation_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->nokRelation)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('nok_postal_address')); ?>:
	<?php echo GxHtml::encode($data->nok_postal_address); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('nok_postcode')); ?>:
	<?php echo GxHtml::encode($data->nok_postcode); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('nok_town')); ?>:
	<?php echo GxHtml::encode($data->nok_town); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('nok_mobile')); ?>:
	<?php echo GxHtml::encode($data->nok_mobile); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('nok_email')); ?>:
	<?php echo GxHtml::encode($data->nok_email); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('disability_id')); ?>:
		<?php echo GxHtml::encode(GxHtml::valueEx($data->disability)); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('occupation')); ?>:
	<?php echo GxHtml::encode($data->occupation); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('employer')); ?>:
	<?php echo GxHtml::encode($data->employer); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('employer_address')); ?>:
	<?php echo GxHtml::encode($data->employer_address); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('employer_telephone')); ?>:
	<?php echo GxHtml::encode($data->employer_telephone); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('employer_email')); ?>:
	<?php echo GxHtml::encode($data->employer_email); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('employer_otherinfo')); ?>:
	<?php echo GxHtml::encode($data->employer_otherinfo); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('extra_info')); ?>:
	<?php echo GxHtml::encode($data->extra_info); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('status')); ?>:
	<?php echo GxHtml::encode($data->status); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('date_modified')); ?>:
	<?php echo GxHtml::encode($data->date_modified); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('expected_completion_date')); ?>:
	<?php echo GxHtml::encode($data->expected_completion_date); ?>
	<br />
	<?php echo GxHtml::encode($data->getAttributeLabel('photo')); ?>:
	<?php echo GxHtml::encode($data->photo); ?>
	<br />
	*/ ?>

</div>